...
routes happy patients (4–5★) to Google reviews and intercepts unhappy ones (1–3★) privately,
emailing the practice manager so a bad public review never gets posted. Clinics get a
read-only analytics dashboard; the founder gets an admin panel for manual onboarding.
...
| QR codes | `qrcode` (in-app, admin panel) | Download PNG per clinic |
...
### Routes
- `/[slug]` — public patient star-rating form
- `/[slug]/sorry` — negative (1–3★) private contact capture
- `/[slug]/ty` — generic thank-you
- `/login` — clinic dashboard login
- `/dashboard` — clinic analytics (auth, owner-scoped)
- `/admin` — founder-only onboarding + QR generation
...
- A Google Cloud project with **Places API (New)** enabled (optional, for live rating)
...
Run the SQL in `supabase/migrations/` **in order** (0001 → 0004). Either:
...
This creates `clinics` + `submissions`, RLS policies, the public RPCs, and the
12-month retention function.
...
## Out of V1 scope (V2)
Twilio SMS, self-serve signup, Stripe billing, PowerSync offline, doctor-specific ratings,
Doctify, founder analytics dashboard, mobile app, white-label.
